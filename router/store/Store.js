...
const sendWhatsappMessage = (order) => {
  client.messages
    .create({
      body: `You have revieved an order of ${order.items.map(
        (item) => item.name + "(" + item.price + ")"
      )} items from ${order.user.username} at Rs${
        order.grand_total
      }.Pls. get it deliver at Address : ${order.user.address}  `,
...
router.get('/',(req,res)=>{
  return res.json({message:"Welcome to MuggleMarket Backend",version:"1.0.0"})
})
    ...
    .then((customer) => {
      stripe.charges
        ...
        .then((result) => {
          return res
            .status(200)
            .json({ message: "Successfull", result: result });
        })
        .catch((error) => {
          console.log(error);
        });
